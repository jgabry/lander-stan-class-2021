...
```{r utility}
utility <- sweep(total_losses, MARGIN = 2, STATS = costs, FUN = '-')
```
...
```{r utility-curves, fig.width = 8, fig.height = 8}
...
utility_df <-
  data.frame(
    utility = as.vector(mean_utility),
...
dec_df <- utility_df %>%
  group_by(building) %>%
  summarise(
    arg_max = which.max(utility) - 1,
    max = max(utility)
  )

ggplot(data = utility_df, aes(x = traps, y = utility)) +
...
* If we wanted to maximize expected utility, we can take expectations at each
station count for each building, and choose the trap number that maximizes
expected revenue for that building. This will be called a maximum revenue
strategy. How can we generate the distribution of portfolio revenue (i.e. the
sum of revenue across all the buildings) under the maximum revenue strategy from
the predictions we already have?
