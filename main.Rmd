...
```{r fit_multi_pois}
fit_multi_pois <- comp_multi_pois$sample(data = standata_simple,
                                         refresh = 0) # turn off printed progress updates
```
```{r fit_multi_pois-summary}
fit_multi_pois$summary()
```
...
median_loss <- t(apply(total_losses, c(1, 2), median))
...
this section explains how to implement a GP as an alternative to the AR(1). 
